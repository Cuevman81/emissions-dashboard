...
//   node --env-file=.env.local scratch/check_pascagoula.mjs
const AQS_EMAIL = process.env.AQS_EMAIL;
const AQS_KEY = process.env.AQS_KEY;
if (!AQS_EMAIL || !AQS_KEY) {
  console.error('AQS_EMAIL and AQS_KEY must be set, e.g. node --env-file=.env.local scratch/check_pascagoula.mjs');
  process.exit(1);
}
